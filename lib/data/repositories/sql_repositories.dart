...
class SqlCardRepository implements CardRepository {
  ...
  Future<TemporalCard?> findById(String id) async {
    final rows = await _db
        .query('temporal_cards', where: 'id = ?', whereArgs: [id], limit: 1);
    return rows.isEmpty ? null : _fromRow(rows.first);
  }

  @override
  Future<void> save(TemporalCard c) => _db.insert(
        'temporal_cards',
        {
          'id': c.id,
          'title': c.title,
...
class SqlAssetRepository implements AssetRepository {
  ...
  Future<SourceAsset?> findById(String id) async {
    final rows = await _db
        .query('source_assets', where: 'id = ?', whereArgs: [id], limit: 1);
    return rows.isEmpty ? null : _fromRow(rows.first);
  }
  ...
  Future<SourceAsset?> findBySha256(String sha256) async {
    final rows = await _db.query(
      'source_assets', where: 'sha256 = ?', whereArgs: [sha256], limit: 1,
    );
    return rows.isEmpty ? null : _fromRow(rows.first);
  }

  @override
  Future<void> save(SourceAsset a) => _db.insert(
        'source_assets',
        {
          'id': a.id,
...
class SqlOcrBlockRepository implements OcrBlockRepository {
  ...
  Future<void> saveAll(String cardId, List<OcrBlock> blocks) =>
      _db.transaction((txn) async {
        await txn
            .delete('ocr_blocks', where: 'card_id = ?', whereArgs: [cardId]);
...
class SqlReminderRepository implements ReminderRepository {
  ...
  Future<List<ReminderPlan>> plansByCard(String cardId) async {
    final rows = await _db
        .query('reminder_plans', where: 'card_id = ?', whereArgs: [cardId]);
    return [
      for (final r in rows)
        ReminderPlan(
          id: r['id']! as String,
  ...
  Future<void> savePlans(String cardId, List<ReminderPlan> plans) =>
      _db.transaction((txn) async {
        await txn.delete(
          'reminder_plans', where: 'card_id = ?', whereArgs: [cardId],
        );
  ...
  Future<void> replaceInstances(
    ...
    List<ReminderInstance> instances,
      ...
      _db.transaction((txn) async {
        await txn.delete(
          'reminder_instances', where: 'card_id = ?', whereArgs: [cardId],
        );
  ...
  ReminderInstance _instanceFromRow(Map<String, Object?> r) =>
      ReminderInstance(
        id: r['id']! as String,
        cardId: r['card_id']! as String,
