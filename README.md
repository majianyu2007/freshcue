...
FreshCue 把截图中的短期信息（活动、取件码、票务、截止事项等）转换为可编辑的时效卡片，并在端侧完成 OCR、中文时间解析、提醒和到期归档。应用不要求账号；除用户主动下载离线识别组件外，不使用网络。
...
- 自动化测试 174 项通过、静态分析 0 issues；原生 det → rec smoke gate 的 10 类合成截图通过 9 类（见 `docs/testing.md`）。
- API 24 模拟器已自动测试界面、图库快捷导入和服务卡片跳转；OCR、分享、提醒、SQLite 持久化等真机行为仍待设备验证。
- 实况窗未实现：本机 SDK 表明第三方普通应用不能直接创建系统实况窗，项目已删除不可用接口和占位实现。

## 界面预览

以下画面来自 HarmonyOS API 24 模拟器，仅用于界面展示，不代表真机能力验证。
...
<table>
  ...
  <tr>
    <td><img src="docs/screenshots/home.jpeg" alt="截期首页" width="240"></td>
    <td><img src="docs/screenshots/detail.jpeg" alt="时效卡片详情" width="240"></td>
    <td><img src="docs/screenshots/settings.jpeg" alt="截期设置" width="240"></td>
    <td><img src="docs/screenshots/service-card.jpeg" alt="HarmonyOS 桌面服务卡片" width="240"></td>
  </tr>
</table>
...
- 网络仅用于用户主动下载离线 OCR 组件；截图和识别文字不会上传。
